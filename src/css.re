...
type brWidth = [ | `length(length) | `thin | `medium | `thick | `variable(string)];
...
type marginValue = [ | `length(length) | `percent(percent) | `auto | `variable(string)];

type paddingValue = [ | `length(length) | `percent(percent) | `variable(string)];

type borderValue = (option(brWidth), option(brStyle), option(color));

type radiusValue = [ | `length(length) | `percent(percent) | `variable(string)];

type attachment = [ | `fixed | `scroll | `local | `variable(string)];

type clipBox = [ | `borderBox | `paddingBox | `contentBox | `text | `variable(string)];

type backgroundImageValue = [ | `url(string) | `variable(string) ];
